...
describe('DocumentService', () => {
  ...
  describe("createDocument", () => {
    it("should create a new document", async () => {
      ...
      vi.mocked(mockDocumentEncryption.exportKey).mockResolvedValue(mockKeyString)
      ...
      expect(mockKeyStorage.saveKey).toHaveBeenCalledWith(result.id, mockKeyString)
    })

    it("should throw error when key generation fails", async () => {
      vi.mocked(mockDocumentEncryption.generateKey).mockRejectedValue(new Error("Key generation failed"))

      await expect(documentService.createDocument()).rejects.toThrow("Key generation failed")
    })
    ...
    it("should throw error when key storage fails", async () => {
      ...
      vi.mocked(mockDocumentEncryption.generateKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.exportKey).mockResolvedValue(mockKeyString)
      vi.mocked(mockKeyStorage.saveKey).mockRejectedValue(new Error("Storage failed"))

      await expect(documentService.createDocument()).rejects.toThrow("Storage failed")
    })
  })
  ...
  describe("loadDocument", () => {
    it("should load document from share URL", async () => {
      ...
      vi.mocked(mockDocumentEncryption.decrypt).mockResolvedValue(JSON.stringify(mockDocumentData))
      ...
      expect(mockDocumentEncryption.importKey).toHaveBeenCalledWith(mockKeyString)
      expect(mockDocumentEncryption.decrypt).toHaveBeenCalledWith("", mockKey)
    })

    it("should throw error for invalid share URL", async () => {
      await expect(documentService.loadDocument("invalid-url")).rejects.toThrow("Invalid share URL")
    })

    it("should throw error when key import fails", async () => {
      const shareUrl = "https://example.com/editor?doc=doc-123&key=key-456"
      
      vi.mocked(mockDocumentEncryption.importKey).mockRejectedValue(new Error("Key import failed"))

      await expect(documentService.loadDocument(shareUrl)).rejects.toThrow("Key import failed")
    })
    ...
    it("should throw error when decryption fails", async () => {
      ...
      vi.mocked(mockDocumentEncryption.importKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.decrypt).mockRejectedValue(new Error("Decryption failed"))

      await expect(documentService.loadDocument(shareUrl)).rejects.toThrow("Decryption failed")
    })
    ...
    it("should throw error for invalid JSON data", async () => {
      ...
      vi.mocked(mockDocumentEncryption.importKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.decrypt).mockResolvedValue("invalid-json")
  ...
  describe("updateDocument", () => {
    it("should update document and send to server", async () => {
      ...
      vi.mocked(mockDocumentEncryption.importKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.encrypt).mockResolvedValue(mockEncryptedContent)
      ...
      expect(mockSyncClient.sendUpdate).toHaveBeenCalledWith(mockEncryptedContent, expect.any(Number))
    })
    ...
    it("should throw error when key not found", async () => {
      ...
      await expect(documentService.updateDocument(document, "New text")).rejects.toThrow("Key not found")
    })
    ...
    it("should throw error when key import fails", async () => {
      ...
      await expect(documentService.updateDocument(document, "New text")).rejects.toThrow("Key import failed")
    })
    ...
    it("should throw error when encryption fails", async () => {
      ...
      vi.mocked(mockKeyStorage.loadKey).mockResolvedValue(mockKeyString)
      vi.mocked(mockDocumentEncryption.importKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.encrypt).mockRejectedValue(new Error("Encryption failed"))

      await expect(documentService.updateDocument(document, "New text")).rejects.toThrow("Encryption failed")
    })
    ...
    it("should throw error when server sync fails", async () => {
      ...
      vi.mocked(mockDocumentEncryption.importKey).mockResolvedValue(mockKey)
      vi.mocked(mockDocumentEncryption.encrypt).mockResolvedValue(mockEncryptedContent)
      vi.mocked(mockSyncClient.sendUpdate).mockRejectedValue(new Error("Sync failed"))

      await expect(documentService.updateDocument(document, "New text")).rejects.toThrow("Sync failed")
    })
  ...
  describe("shareDocument", () => {
    ...
    it("should throw error when key not found", async () => {
      ...
      await expect(documentService.shareDocument("doc-123")).rejects.toThrow("Key not found")
    })

    it("should throw error when key loading fails", async () => {
      vi.mocked(mockKeyStorage.loadKey).mockRejectedValue(new Error("Storage read failed"))

      await expect(documentService.shareDocument("doc-123")).rejects.toThrow("Storage read failed")
    })
  })
})
